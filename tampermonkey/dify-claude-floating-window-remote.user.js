...
//   2) @match 宽化为 http://*/* — 之前 http://REDACTED_HOST:9980/* 是占位符，
//      用户在公网（端口 9980 路由器转发）测试时脚本压根没激活（match 不到）
//   3) BRIDGE_CANDIDATES 占位符 __REMOTE_BRIDGE_HOST__ — 公网 IP 不入仓
//      用户在 Tampermonkey 脚本值里 GM_setValue("__bridge_remote_host__", "<公网 IP>")
...
// ★ 0.3.11-remote 自动从 window.location 推导 remote bridge host：
//   之前要求用户在油猴脚本值（Values 标签）里设 __bridge_remote_host__，
//   但油猴面板只有"编辑器/设置"两个标签，多数用户找不到。
...
(function () {
  ...
  //   - 公网 IP 仍不入仓（避免脱敏负担），用户在 Dify 页面访问就会自动生效
  //   - 本机访问（127.0.0.1/localhost/192.168.x.x）仍走 loopback fallback
  // ★ 0.3.8-remote: 用户也可在 Tampermonkey 脚本值里设
  ...
  // ★ 0.3.11-remote: 优先 GM 值；否则从 window.location 自动推导；都无则降级跳过占位符
    ...
    try {
      ...
      if (!h || !h.trim()) {
        ...
        try {
          ...
          // 只在 "非 loopback / 非纯 LAN 192.168.x.x" 时才算 remote（即真的公网 / 域名）
      ...
      const opts = Object.assign(
        {
          ...
          onerror: function (err) {
            // ★ 0.3.8-remote: GM_xmlhttpRequest 传 raw err（{error, status} 对象），
            //   直接 reject 会让上游 e.message || e 走到 e 分支 → toString "[object Object]"
            //   包成 Error 让上游能拿到 .message
            // ★ 0.3.13-remote: 加 sentinel — 让 unhandledrejection 监听器识别这是我们自己的错
  ...
  function injectUI() {
    ...
    if (_userPopoverShadowHandler) {
      shadowRoot && shadowRoot.removeEventListener("mousedown", _userPopoverShadowHandler, true);
      _userPopoverShadowHandler = null;
    }
    if (_userPopoverDocHandler) {
      document.removeEventListener("mousedown", _userPopoverDocHandler, true);
      _userPopoverDocHandler = null;
    }
  ...
  // ★ 0.3.13-remote: 仅归因 plugin 自身的 sentinel-标记错误 —— 页面原生 JS 报错
  ...
  const STYLES = `
    ...
    /* ★ 0.3.7-remote: 第二行状态栏 —— 拟终端 prompt 链 */
    .dcfw-statusbar {
    ...
    .dcfw-statusbar-prompt { opacity: 0.55; font-weight: 700; flex: 0 0 auto; }
    .dcfw-statusbar-label { opacity: 0.55; font-weight: 400; }
    .dcfw-statusbar-value { opacity: 0.95; font-weight: 600; min-width: 0; }
    ...
    .dcfw-statusbar-cell .dcfw-bridge-badge {
      ...
      font-weight: 600; color: inherit; border: none; margin-left: 0; cursor: pointer;
    }
    ...
    .dcfw-statusbar-cell .dcfw-bridge-dot { width: 7px; height: 7px; }
    ...
    /* ★ 0.3.8-remote: tab 不再带 ▎ 前缀 — 用户反馈图标化误以为是 icon */
  ...
  function toggleUserPopover(show) {
    ...
    if (shouldShow) {
      renderUserPopover();
      // ★ 0.3.4-remote v2: 清理上次监听器 + 用双 listener 替代 composedPath
      _cleanupUserPopoverListeners();
      setTimeout(() => {
        // Tier 1: shadowRoot capture — shadow 内部无 retarget，e.target 就是真实元素
        _userPopoverShadowHandler = (e) => {
          if (e.target && e.target.closest && e.target.closest("#dcfw-user-badge, .dcfw-user-popover")) return;
          toggleUserPopover(false);
        };
        shadowRoot.addEventListener("mousedown", _userPopoverShadowHandler, true);
        // Tier 2: document — 面板外点击用 hostEl.contains（比 composedPath 可靠）
        _userPopoverDocHandler = (e) => {
          // shadow 内部事件 retarget 到 hostEl → contains=true，不关（交给 Tier 1）
          if (hostEl && hostEl.contains(e.target)) return;
          toggleUserPopover(false);
        };
        document.addEventListener("mousedown", _userPopoverDocHandler, true);
      }, 0);
    } else {
      _cleanupUserPopoverListeners();
    }
  }
  function _cleanupUserPopoverListeners() {
    if (_userPopoverShadowHandler) {
      shadowRoot && shadowRoot.removeEventListener("mousedown", _userPopoverShadowHandler, true);
      _userPopoverShadowHandler = null;
    }
  ...
  let _userPopoverShadowHandler = null;  // ★ 0.3.4-remote v2: shadowRoot capture（无 retarget）
  let _userPopoverDocHandler = null;     // ★ 0.3.4-remote v2: document capture（hostEl.contains 兜底）
