...
def test_local_fix() -> list[str]:
    ...
    # 7) v0.3.7 标题栏拆两行 + 恢复原始机器人图样（对照桌面 CLAUDECODE bot 文档）
    failures += assert_contains("local.changelog_v037", src, "★ 0.3.7 标题栏拆两行 + 恢复原始机器人图样")
    failures += assert_contains("local.robot_original_art", src, 'btn.innerHTML = \'<pre class="dcfw-fab-robot" aria-hidden="true">  ▐▛███▜▌\\n▝▜█████▛▘\\n     ▘▘ ▝▝</pre>\'')
...
def test_remote_fix() -> list[str]:
    ...
    # v0.3.7-remote 标题栏拆两行 + 恢复原始机器人图样
    failures += assert_contains("remote.changelog_v037", src, "★ 0.3.7-remote 标题栏拆两行 + 恢复原始机器人图样")
    failures += assert_contains("remote.robot_original_art", src, 'btn.innerHTML = \'<pre class="dcfw-fab-robot" aria-hidden="true">  ▐▛███▜▌\\n▝▜█████▛▘\\n     ▘▘ ▝▝</pre>\'')
